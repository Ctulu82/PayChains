...
impl Pubkey {
    ...
    ///
    /// [pda]: https://docs.paychains.com/developing/programming-model/calling-between-programs#program-derived-addresses
    ///
    ...
    /// as a PayChains `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain PayChains program must be
    ...
    /// program, which invokes an on-chain PayChains program that uses the address
    ...
    /// instruction as a PayChains transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain PayChains program:
    ///
    /// ```
    ...
    ///
    /// ```ignore
    /// # // NB: This example depends on paychains_sdk and paychains_client, and adding
    ...
    /// # use paychains_program::instruction::Instruction;
    /// # use paychains_program::hash::Hash;
    /// # use paychains_program::instruction::AccountMeta;
    /// # use paychains_program::system_program;
    /// # use paychains_sdk::signature::Keypair;
    /// # use paychains_sdk::signature::{Signer, Signature};
    /// # use paychains_sdk::transaction::Transaction;
    ...
    pub fn try_find_program_address(seeds: &[&[u8]], program_id: &Pubkey) -> Option<(Pubkey, u8)> {
        ...
        {
            extern "C" {
                fn pay_try_find_program_address(
                    seeds_addr: *const u8,
                    seeds_len: u64,
    ...
    pub fn log(&self) {
        ...
        {
            extern "C" {
                fn pay_log_pubkey(pubkey_addr: *const u8);
            }
            unsafe { pay_log_pubkey(self.as_ref() as *const _ as *const u8) };
        }

        #[cfg(not(target_arch = "bpf"))]
        crate::program_stubs::pay_log(&self.to_string());
    }
}
