#pragma once
/**
 * @brief PayChains deprecated BPF loader deserializer to be used when deploying
 ...
 * instead it populates the pointers and lengths in PayAccountInfo and data so
 ...
 */
static bool pay_deserialize_deprecated(
  const uint8_t *input,
  PayParameters *params,
  uint64_t ka_num
) {
  ...
  for (int i = 0; i < params->ka_num; i++) {
    ...
    if (dup_info == UINT8_MAX) {
      ...
      // key
      params->ka[i].key = (PayPubkey *) input;
      input += sizeof(PayPubkey);
      ...
      // owner
      params->ka[i].owner = (PayPubkey *) input;
      input += sizeof(PayPubkey);
  ...
  params->program_id = (PayPubkey *) input;
  input += sizeof(PayPubkey);
