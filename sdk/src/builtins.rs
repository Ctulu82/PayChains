//! PayChains builtin helper macros
    ...
    ($name:ident, $id:path, $entrypoint:expr) => {
        ...
        macro_rules! $name {
            () => {
                ...
                // see https://github.com/paychains-labs/paychains/issues/10933.
                // For now, we need to use `::paychains_sdk`
                //
                // `respan!` respans the path `$crate::id`, which we then call (hence the extra
                // parens)
                (
                    stringify!($name).to_string(),
                    ::paychains_sdk::respan!($crate::$id, $name)(),
                    $entrypoint,
                )
            };
        }
...
/// mod item_wrapper {
/// use paychains_sdk::keyed_account::KeyedAccount;
/// use paychains_sdk::instruction::InstructionError;
/// use paychains_sdk::pubkey::Pubkey;
/// use paychains_sdk::declare_builtin;
///
...
///
/// # use item_wrapper::id;
/// assert_eq!(id(), paychains_sdk::system_program::ID);
/// ```
#[macro_export]
