...
//! standard signing sources, from the command line, as in the PayChains CLI.
//!
...
//! sources supported by the PayChains CLI. Many other functions here are
...
impl DefaultSigner {
    ...
    /// use paychains_clap_utils::keypair::DefaultSigner;
    /// use paychains_clap_utils::offline::OfflineArgs;
    ///
            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"paychains-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    /// ```no_run
    /// use clap::{App, Arg, value_t_or_exit};
    /// use paychains_clap_utils::keypair::{DefaultSigner, signer_from_path};
    /// use paychains_clap_utils::offline::OfflineArgs;
    /// use paychains_sdk::signer::Signer;
    ///
    /// let clap_app = App::new("my-program")
    ///     // The argument we'll parse as a signer "path"
    ///     .arg(Arg::with_name("keypair")
    ...
    /// ```no_run
    /// use clap::{App, Arg, value_t_or_exit};
    /// use paychains_clap_utils::keypair::{SignerFromPathConfig, DefaultSigner};
    /// use paychains_clap_utils::offline::OfflineArgs;
    ///
    /// let clap_app = App::new("my-program")
    ///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use paychains_clap_utils::keypair::signer_from_path;
/// use paychains_clap_utils::offline::OfflineArgs;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use paychains_clap_utils::keypair::{signer_from_path_with_config, SignerFromPathConfig};
/// use paychains_clap_utils::offline::OfflineArgs;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{}\". Run \"paychains-keygen new\" to create a keypair file: {}", path, e),
            )
            .into()),
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use paychains_clap_utils::keypair::pubkey_from_path;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::with_name("keypair")
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"paychains-keygen new\" to create a keypair file: {}",
                    path, e
                ),
            )
            .into()),
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use paychains_clap_utils::keypair::keypair_from_path;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::with_name("keypair")
...
mod tests {
    use {
        ...
        paychains_remote_wallet::{locator::Manufacturer, remote_wallet::initialize_wallet_manager},
        paychains_sdk::{signer::keypair::write_keypair_file, system_instruction},
