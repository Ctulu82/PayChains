...
use {
    ...
    paychains_metrics::{inc_new_counter_debug, inc_new_counter_error},
    paychains_net_utils::{
...
const MIN_STAKE_FOR_GOSSIP: u64 = paychains_sdk::native_token::LAMPORTS_PER_PAY;
...
impl ClusterInfo {
    ...
    pub fn get_node_version(&self, pubkey: &Pubkey) -> Option<paychains_version::Version> {
        let gossip_crds = self.gossip.crds.read().unwrap();
    ...
    pub fn gossip(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .unwrap();
        Builder::new()
            .name("paychains-gossip".to_string())
            .spawn(move || {
    ...
    pub(crate) fn listen(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            .num_threads(get_thread_count().min(8))
            .thread_name(|i| format!("pay-gossip-work-{}", i))
            .build()
            .unwrap();
        Builder::new()
            .name("paychains-listen".to_string())
            .spawn(move || {
                while !exit.load(Ordering::Relaxed) {
...
mod tests {
    use {
        ...
        paychains_vote_program::{vote_instruction, vote_state::Vote},
        std::{
            iter::repeat_with,
    ...
    fn test_gossip_node() {
        ...
        let (node, _, _) = ClusterInfo::spy_node(paychains_sdk::pubkey::new_rand(), 0);
    ...
    fn test_handle_pull() {
        ...
        let entrypoint_pubkey = paychains_sdk::pubkey::new_rand();
        ...
        let entrypoint_pubkey2 = paychains_sdk::pubkey::new_rand();
        assert_eq!(
            (1, 0, 0),
    ...
    fn test_cluster_spy_gossip() {
        ...
        let node = Node::new_localhost();
        let (spy, _, _) = ClusterInfo::spy_node(paychains_sdk::pubkey::new_rand(), 0);
        let cluster_info = Arc::new(ClusterInfo::new(
            node.info,
            Arc::new(Keypair::new()),
            SocketAddrSpace::Unspecified,
    ...
    fn test_cluster_info_new() {
        let d = ContactInfo::new_localhost(&paychains_sdk::pubkey::new_rand(), timestamp());
        let cluster_info = ClusterInfo::new(
            d.clone(),
            Arc::new(Keypair::new()),
            SocketAddrSpace::Unspecified,
    ...
    fn new_vote_transaction<R: Rng>(rng: &mut R, slots: Vec<Slot>) -> Transaction {
        let vote = Vote::new(slots, paychains_sdk::hash::new_rand(rng));
        let ix = vote_instruction::vote(
            &Pubkey::new_unique(), // vote_pubkey
            &Pubkey::new_unique(), // authorized_voter_pubkey
            vote,
    ...
    fn test_split_gossip_messages() {
        ...
        let splits: Vec<_> =
            ClusterInfo::split_gossip_messages(PUSH_MESSAGE_MAX_PAYLOAD_SIZE, values.clone())
                .collect();
        let self_pubkey = paychains_sdk::pubkey::new_rand();
    ...
    fn test_pull_from_entrypoint_if_not_present() {
        ...
        let other_node_pubkey = paychains_sdk::pubkey::new_rand();
    ...
    fn test_process_entrypoint_adopt_shred_version() {
        ...
        let mut gossiped_entrypoint1_info =
            ContactInfo::new_localhost(&paychains_sdk::pubkey::new_rand(), timestamp());
