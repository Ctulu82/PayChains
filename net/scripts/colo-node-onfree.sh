...
if [[ -f "${PAYCHAINS_LOCK_FILE}" ]]; then
  ...
  . "${PAYCHAINS_LOCK_FILE}"
  if [[ "${PAYCHAINS_LOCK_USER}" = "${PAYCHAINS_USER}"  || -n "${FORCE_DELETE}" ]]; then
    ...
    git clean -qxdff
    rm -f /paychains-scratch/* /paychains-scratch/.[^.]*
...
EOAK
    ...
    echo "Invalid user: expected \"${PAYCHAINS_LOCK_USER}\" got \"${PAYCHAINS_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
