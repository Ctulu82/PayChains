...
declare -r PAYCHAINS_LOCK_FILE="/home/paychains/.paychains.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l paychains -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
colo_whoami() {
  declare ME LINE PAY_USER EOL
  while read -r LINE; do
    declare IP RC
    IFS=$'\x1f' read -r IP RC PAY_USER EOL <<< "${LINE}"
    if [ "${RC}" -eq 0 ]; then
      [[ "${EOL}" = "EOL" ]] || echo "${FUNCNAME[0]}: Unexpected input \"${LINE}\"" 1>&2
      if [ -z "${ME}" ] || [ "${ME}" = "${PAY_USER}" ]; then
        ME="${PAY_USER}"
      else
        echo "Found conflicting username \"${PAY_USER}\" on ${IP}, expected \"${ME}\"" 1>&2
      fi
    fi
  done < <(colo_instance_run_foreach "[ -n \"\${PAYCHAINS_USER}\" ] && echo -e \"\${PAYCHAINS_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_PAYCHAINS_USER=""
colo_get_paychains_user() {
  if [ -z "${COLO_PAYCHAINS_USER}" ]; then
    COLO_PAYCHAINS_USER=$(colo_whoami)
  fi
  echo "${COLO_PAYCHAINS_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${PAYCHAINS_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${PAYCHAINS_LOCK_FILE}" && flock -s 9 && . "${PAYCHAINS_LOCK_FILE}" && exec 9>&-
  echo -e "\${PAYCHAINS_LOCK_USER}\\x1f\${PAYCHAINS_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
...
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-paychains-user-authorized_keys.sh 2> /dev/null)'
