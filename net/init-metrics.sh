...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and PAYCHAINS_METRICS_CONFIG is
...
host="https://internal-metrics.paychains.com:8086"
...
if $useEnv; then
  [[ -n $PAYCHAINS_METRICS_CONFIG ]] ||
    usage "PAYCHAINS_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  PAYCHAINS_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export PAYCHAINS_METRICS_CONFIG=\"$PAYCHAINS_METRICS_CONFIG\"" >> "$configFile"
