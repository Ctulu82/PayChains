...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip paychains-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'paychains-install deploy ...' for the
...
build() {
  ...
  if [[ $(uname) != Linux || ! " ${supported[*]} " =~ $(lsb_release -sr) ]]; then
    ...
    source "$PAYCHAINS_ROOT"/ci/rust-version.sh
  ...
  (
    ...
    (
      ...
      echo "commit: $COMMIT"
    ) > "$PAYCHAINS_ROOT"/farf/version.yml
  )
  echo "Build took $SECONDS seconds"
}

PAYCHAINS_HOME="\$HOME/paychains"
CARGO_BIN="\$HOME/.cargo/bin"

startCommon() {
  declare ipAddress=$1
  test -d "$PAYCHAINS_ROOT"
  if $skipSetup; then
    # shellcheck disable=SC2029
    ssh "${sshOptions[@]}" "$ipAddress" "
      set -x;
      mkdir -p $PAYCHAINS_HOME/config;
      rm -rf ~/config;
      mv $PAYCHAINS_HOME/config ~;
      rm -rf $PAYCHAINS_HOME;
      mkdir -p $PAYCHAINS_HOME $CARGO_BIN;
      mv ~/config $PAYCHAINS_HOME/
    "
  else
    # shellcheck disable=SC2029
    ssh "${sshOptions[@]}" "$ipAddress" "
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "paychains@$ipAddress"
...
syncScripts() {
  ...
  rsync -vPrc -e "ssh ${sshOptions[*]}" \
    --exclude 'net/log*' \
    "$PAYCHAINS_ROOT"/{fetch-perf-libs.sh,fetch-spl.sh,scripts,net,multinode-demo} \
    "$ipAddress":"$PAYCHAINS_HOME"/ > /dev/null
}
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PAYCHAINS_ROOT"/paychains-release/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PAYCHAINS_ROOT"/paychains-release/version.yml "$ipAddress:~/"
    ;;
  local)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PAYCHAINS_ROOT"/farf/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PAYCHAINS_ROOT"/farf/version.yml "$ipAddress:~/"
    ;;
  skip)
    ;;
  *)
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./paychains/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startNode() {
  ...
  (
    ...
    if [[ $nodeType = blockstreamer ]] && [[ -n $letsEncryptDomainName ]]; then
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "sudo -H /certbot-restore.sh $letsEncryptDomainName maintainers@paychains.foundation"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./paychains/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "paychains-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./paychains/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./paychains/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying paychains-install update: $updatePlatform"
    (
      set -x

      scripts/paychains-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:paychains/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./paychains/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$PAYCHAINS_ROOT"/paychains-release.tar.bz2
      declare updateDownloadUrl=https://release.paychains.com/"$releaseChannel"/paychains-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -L -I "$updateDownloadUrl"
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$PAYCHAINS_ROOT"/paychains-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$PAYCHAINS_ROOT"/paychains-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$PAYCHAINS_ROOT"/paychains-release
      cd "$PAYCHAINS_ROOT"; tar jfxv "$tarballFilename"
      cat "$PAYCHAINS_ROOT"/paychains-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$PAYCHAINS_ROOT"/paychains-release/version.yml; then
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./paychains/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$PAYCHAINS_ROOT"/paychains-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./paychains/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":paychains/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        "$here"/scp.sh "$netemConfigFile" paychains@"$ipAddress":"$PAYCHAINS_HOME"
      done
    fi
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh paychains@"${validatorIpList[$i]}" 'paychains/scripts/net-shaper.sh' \
      "$netemCommand" ~paychains/paychains/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh paychains@"$ipAddress" 'paychains/scripts/netem.sh delete < paychains/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh paychains@"${validatorIpList[$i]}" "echo $netemConfig > paychains/netem.cfg; paychains/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
