...
cat >> ~/paychains/on-reboot <<EOF
  ...
  sudo RUST_LOG=info ~paychains/.cargo/bin/paychains-sys-tuner --user $(whoami) > sys-tuner.log 2>&1 &
  echo \$! > sys-tuner.pid
  ...
  if ${GPU_CUDA_OK} && [[ -e /dev/nvidia0 ]]; then
    echo Selecting paychains-validator-cuda
    export PAYCHAINS_CUDA=1
  elif ${GPU_FAIL_IF_NONE} ; then
    echo "Expected GPU, found none!"
    export PAYCHAINS_GPU_MISSING=1
  fi
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$PAYCHAINS_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          paychains-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            paychains-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(paychains-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        paychains-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(paychains-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(paychains-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(paychains-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        paychains-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      paychains-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(paychains-ledger-tool -l config/bootstrap-validator bank-hash)
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/paychains-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/paychains/config/blockstreamer-identity.json "$PAYCHAINS_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/paychains/config/validator-identity-"$nodeIndex".json "$PAYCHAINS_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/paychains/config/validator-stake-"$nodeIndex".json "$PAYCHAINS_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/paychains/config/validator-vote-"$nodeIndex".json "$PAYCHAINS_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/paychains/config/shred-version "$PAYCHAINS_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/paychains/config/bank-hash "$PAYCHAINS_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/paychains/config/faucet.json "$PAYCHAINS_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$PAYCHAINS_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/paychains-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$PAYCHAINS_CONFIG_DIR"/validator-identity.json ]]; then
      paychains-keygen new --no-passphrase -so "$PAYCHAINS_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$PAYCHAINS_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$PAYCHAINS_CONFIG_DIR"/vote-account.json ]]; then
      paychains-keygen new --no-passphrase -so "$PAYCHAINS_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$PAYCHAINS_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$PAYCHAINS_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$PAYCHAINS_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/paychains/config/faucet.json "$PAYCHAINS_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      paychains --url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$PAYCHAINS_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$PAYCHAINS_CONFIG_DIR"/stake-account.json \
