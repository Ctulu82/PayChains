...
use {
    ...
    paychains_cli_output::display::format_labeled_address,
    paychains_client::{client_error, rpc_client::RpcClient, rpc_response::RpcVoteAccountStatus},
    paychains_metrics::{datapoint_error, datapoint_info},
    paychains_notifier::Notifier,
    paychains_sdk::{
        hash::Hash,
        native_token::{pay_to_lamports, Pay},
        pubkey::Pubkey,
    },
    std::{
        collections::HashMap,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `paychains-watchtower`:
        ...
        define environment variable before running `paychains-watchtower`:
        ...
        .arg(
            Arg::with_name("minimum_validator_identity_balance")
                ...
                .help("Alert when the validator identity balance is less than this amount of PAY")
        )
        .arg(
    ...
    let minimum_validator_identity_balance = pay_to_lamports(value_t_or_exit!(
        matches,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    paychains_metrics::set_panic_hook("watchtower");
    ...
    loop {
        let failure = match get_cluster_info(&config, &rpc_client) {
            Ok((transaction_count, recent_blockhash, vote_accounts, validator_balances)) => {
                ...
                info!(
                    ...
                    Pay(total_stake),
                    Pay(total_current_stake),
                    Pay(total_delinquent_stake)
                );
                ...
                for validator_identity in config.validator_identity_pubkeys.iter() {
                    ...
                    if let Some(balance) = validator_balances.get(validator_identity) {
                        if *balance < config.minimum_validator_identity_balance {
                            failures.push((
                                "balance",
                                format!("{} has {}", formatted_validator_identity, Pay(*balance)),
                            ));
                        }
                    }
                }
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(&format!("paychains-watchtower: {}", all_clear_msg));
            }
