...
fn tune_poh_service_priority(uid: u32) {
    ...
    if let Some(pid) = find_pid("paychains-validato", "/proc", uid, |dir| {
        let mut path = dir.path();
        path.push("task");
        find_pid("paychains-poh-serv", path, uid, |dir1| {
...
fn main() {
    ...
    if let Err(e) = std::fs::remove_file(paychains_sys_tuner::PAYCHAINS_SYS_TUNER_PATH) {
        if e.kind() != std::io::ErrorKind::NotFound {
            panic!("Failed to remove stale socket file: {:?}", e)
        }
    }

    let listener = unix_socket::UnixListener::bind(paychains_sys_tuner::PAYCHAINS_SYS_TUNER_PATH)
    ...
    if let Some(user) = users::get_user_by_name(&user) {
        peer_uid = user.uid();
        info!("UID for paychains is {}", peer_uid);
        nix::unistd::chown(
            paychains_sys_tuner::PAYCHAINS_SYS_TUNER_PATH,
