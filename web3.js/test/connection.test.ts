...
import {Token, u64} from '@paychains/spl-token';
...
describe('Connection', () => {
        ...
        value: [
          {
            account: {
              data: ['', 'base64'],
              executable: false,
              lamports: LAMPORTS_PER_PAY - feeCalculator.lamportsPerSignature,
          ...
          },
          {
            account: {
              ...
              executable: false,
              lamports:
                0.5 * LAMPORTS_PER_PAY - feeCalculator.lamportsPerSignature,
              owner: programId.publicKey.toBase58(),
              rentEpoch: 20,
            },
      ...
      programAccounts.forEach(function (keyedAccount) {
        if (keyedAccount.pubkey.equals(account0.publicKey)) {
          expect(keyedAccount.account.lamports).to.eq(
            LAMPORTS_PER_PAY - feeCalculator.lamportsPerSignature,
          );
        } else {
  ...
  it('getVersion', async () => {
    ...
    expect(version['paychains-core']).to.be.ok;
  });
  ...
  it('request airdrop', async () => {
    ...
    expect(accountInfo.lamports).to.eq(LAMPORTS_PER_PAY);
    ...
    expect(parsedAccountInfo.lamports).to.eq(LAMPORTS_PER_PAY);
  ...
  if (process.env.TEST_LIVE) {
    ...
    it('multi-instruction transaction', async () => {
      ...
      // accountFrom may have less than LAMPORTS_PER_PAY due to transaction fees
      expect(
    ...
    it('https request', async () => {
      const connection = new Connection('https://api.mainnet-beta.paychains.com');
      const version = await connection.getVersion();
      expect(version['paychains-core']).to.be.ok;
    });
  }
