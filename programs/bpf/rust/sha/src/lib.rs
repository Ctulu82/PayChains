...
use paychains_program::{custom_panic_default, msg};

fn test_sha256_hasher() {
    use paychains_program::hash::{hashv, Hasher};
...
fn test_keccak256_hasher() {
    use paychains_program::keccak::{hashv, Hasher};
    let vals = &["Gaggablaghblagh!".as_ref(), "flurbos".as_ref()];
    let mut hasher = Hasher::default();
    hasher.hashv(vals);
    assert_eq!(hashv(vals), hasher.result());
...
fn test_blake3_hasher() {
    use paychains_program::blake3::hashv;
