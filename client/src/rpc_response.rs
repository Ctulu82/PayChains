use {
    crate::client_error,
    paychains_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    paychains_sdk::{
...
pub struct RpcVersionInfo {
    /// The current version of paychains-core
    pub paychains_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.paychains_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.paychains_core.split_whitespace().next() {
