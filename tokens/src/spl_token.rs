use {
    ...
    paychains_sdk::{instruction::Instruction, message::Message, native_token::lamports_to_pay},
    paychains_transaction_status::parse_token::spl_token_instruction,
...
pub fn check_spl_token_balances(
    ...
    if fee_payer_balance < fees + account_creation_amount {
        return Err(Error::InsufficientFunds(
            ...
            lamports_to_pay(fees + account_creation_amount).to_string(),
        ));
    }
...
mod tests {
    ...
    // BanksClient into the `paychains-tokens` methods. With the revert to RpcClient in this module
    // (https://github.com/paychains-labs/paychains/pull/13623), that approach was no longer viable.
    ...
    //
    // https://github.com/paychains-labs/paychains/blob/5511d52c6284013a24ced10966d11d8f4585799e/tokens/src/spl_token.rs#L490-L685
}
