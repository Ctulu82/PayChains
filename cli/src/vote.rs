use {
    ...
    paychains_sdk::{
        ...
        native_token::lamports_to_pay, pubkey::Pubkey, system_instruction::SystemError,
            ...
            SubCommand::with_name("withdraw-from-vote-account")
                ...
                .arg(
                    pubkey!(Arg::with_name("destination_account_pubkey")
                        ...
                        .required(true),
                        "The recipient of withdrawn PAY. "),
                )
                .arg(
                    Arg::with_name("amount")
                        ...
                        .help("The amount to withdraw, in PAY; accepts keyword ALL, which for this command means account balance minus rent-exempt minimum"),
                )
                .arg(
                    Arg::with_name("authorized_withdrawer")
                        .long("authorized-withdrawer")
            ...
            SubCommand::with_name("close-vote-account")
                ...
                .arg(
                    pubkey!(Arg::with_name("destination_account_pubkey")
                        ...
                        .value_name("RECIPIENT_ADDRESS")
                        .required(true),
                        "The recipient of all withdrawn PAY. "),
                )
                .arg(
                    Arg::with_name("authorized_withdrawer")
                        .long("authorized-withdrawer")
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &paychains_vote_program::id())?
    } else {
        vote_account_pubkey
    };
    ...
    if !sign_only {
            ...
            rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
        {
            if let Some(vote_account) = response.value {
                let err_msg = if vote_account.owner == paychains_vote_program::id() {
    ...
    if !sign_only {
        ...
        if let SpendAmount::Some(withdraw_amount) = withdraw_amount {
            ...
            if balance_remaining < minimum_balance && balance_remaining != 0 {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} PAY to remain rent exempt", lamports_to_pay(minimum_balance)
                ))
                .into());
            }
        }
...
mod tests {
    use {
        ...
        paychains_client::blockhash_query,
        paychains_sdk::{
            hash::Hash,
    ...
    fn test_parse_command() {
        ...
        let authed = paychains_sdk::pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
