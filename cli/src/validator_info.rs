use {
    ...
    paychains_account_decoder::validator_info::{
    ...
    paychains_cli_output::{CliValidatorInfo, CliValidatorInfoVec},
    paychains_client::rpc_client::RpcClient,
    paychains_config_program::{config_instruction, get_config_data, ConfigKeys, ConfigState},
...
fn verify_keybase(
    ...
    if let Some(keybase_username) = keybase_username.as_str() {
        let url = format!(
            "https://keybase.pub/{}/paychains/validator-{:?}",
...
fn parse_validator_info(
    ...
    if account.owner != paychains_config_program::id() {
    ...
    fn validator_info_subcommands(self) -> Self {
        self.subcommand(
            SubCommand::with_name("validator-info")
                .about("Publish/get Validator info on PayChains")
                .setting(AppSettings::SubcommandRequiredElseHelp)
                .subcommand(
                    SubCommand::with_name("publish")
                        .about("Publish Validator info on PayChains")
                        .arg(
                ...
                .subcommand(
                    SubCommand::with_name("get")
                        .about("Get and parse PayChains Validator info")
                        .arg(
                            Arg::with_name("info_pubkey")
                                .index(1)
                                .value_name("PUBKEY")
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&paychains_config_program::id())?;
...
mod tests {
    ...
    fn test_parse_validator_info() {
        ...
        assert_eq!(
            parse_validator_info(
                &Pubkey::default(),
                &Account {
                    owner: paychains_config_program::id(),
                    data,
                    ..Account::default()
                }
            )
    ...
    fn test_parse_validator_info_not_validator_info_account() {
        assert!(parse_validator_info(
            &Pubkey::default(),
            &Account {
                owner: paychains_sdk::pubkey::new_rand(),
                ..Account::default()
            }
        )
        .unwrap_err()
