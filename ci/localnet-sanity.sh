...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $paychains_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $paychains_validator --ledger "$PAYCHAINS_CONFIG_DIR"/bootstrap-validator exit --force || true
  $paychains_validator --ledger "$PAYCHAINS_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $paychains_ledger_tool --ledger "$PAYCHAINS_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $paychains_keygen new --no-passphrase -fso $client_keypair || exit $?
    $paychains_gossip spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
