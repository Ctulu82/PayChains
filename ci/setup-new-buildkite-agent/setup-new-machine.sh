...
cat >/etc/apt/apt.conf.d/99-paychains <<'EOF'
...
"$PAYCHAINS_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$PAYCHAINS_ROOT"/net/scripts/install-certbot.sh
...
"$PAYCHAINS_ROOT"/net/scripts/install-earlyoom.sh
"$PAYCHAINS_ROOT"/net/scripts/install-nodejs.sh
"$PAYCHAINS_ROOT"/net/scripts/localtime.sh
"$PAYCHAINS_ROOT"/net/scripts/install-redis.sh
"$PAYCHAINS_ROOT"/net/scripts/install-rsync.sh
"$PAYCHAINS_ROOT"/net/scripts/install-libssl-compatability.sh
