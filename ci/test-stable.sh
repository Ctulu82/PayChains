...
case $testName in
test-stable)
  _ "$cargo" stable test --jobs "$NPROC" --all --exclude paychains-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-bpf)
  ...
  # paychains-keygen required when building C programs
  ...
  # BPF paychains-sdk legacy compile test
...
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for paychains-core unit tests
  ...
  _ "$cargo" stable test --package paychains-perf --package paychains-ledger --package paychains-core --lib ${V:+--verbose} -- --nocapture
  _ "$cargo" stable run --manifest-path poh-bench/Cargo.toml ${V:+--verbose} -- --hashes-per-tick 10
  ;;
test-local-cluster)
  _ "$cargo" stable build --release --bins ${V:+--verbose}
  _ "$cargo" stable test --release --package paychains-local-cluster --test local_cluster ${V:+--verbose} -- --nocapture --test-threads=1
  exit 0
  ;;
test-local-cluster-flakey)
  _ "$cargo" stable build --release --bins ${V:+--verbose}
  _ "$cargo" stable test --release --package paychains-local-cluster --test local_cluster_flakey ${V:+--verbose} -- --nocapture --test-threads=1
  exit 0
  ;;
test-local-cluster-slow)
  _ "$cargo" stable build --release --bins ${V:+--verbose}
  _ "$cargo" stable test --release --package paychains-local-cluster --test local_cluster_slow ${V:+--verbose} -- --nocapture --test-threads=1
  exit 0
  ;;
test-wasm)
  _ node --version
