---
title: Add PayChains to Your Exchange
---

This guide describes how to add PayChains's native token PAY to your cryptocurrency
exchange.
...
- to have a self-administered gateway to the PayChains mainnet-beta cluster to get
...
PayChains nodes demand relatively high computing power to handle our fast blocks
and high TPS. For specific requirements, please see
[hardware recommendations](../running-validator/validator-reqs.md).

To run an api node:

1. [Install the PayChains command-line tool suite](../cli/install-paychains-cli-tools.md)
...
[Current parameters for Mainnet Beta](../clusters.md#example-paychains-validator-command-line-2)
...
`--limit-ledger-size` if desired. Check `paychains-validator --help` for the
default limit value used by `--limit-ledger-size`. More information about
selecting a custom limit value is [available
here](https://github.com/paychains-labs/paychains/blob/583cec922b6107e0f85c7e14cb5e642bc7dfb340/core/src/ledger_cleanup_service.rs#L15-L26).
...
ensure you miss as little data as possible. Running the paychains software as a
systemd service is one great option.

For monitoring, we provide
[`paychains-watchtower`](https://github.com/paychains-labs/paychains/blob/master/watchtower/README.md),
which can monitor your validator and detect with the `paychains-validator` process
is unhealthy. It can directly be configured to alert you via Slack, Telegram,
Discord, or Twillio. For details, run `paychains-watchtower --help`.

```bash
paychains-watchtower --validator-identity <YOUR VALIDATOR IDENTITY>
```
...
`paychains-validator` command to receive historical ledger data instead of a
snapshot.
...
traffic from all other PayChains validators. While this is the most efficient mode of
operation, and is strongly recommended, it is possible to restrict the
validator to only require inbound traffic from one other PayChains validator.
...
PayChains accounts do not require any on-chain initialization; once they contain
some PAY, they exist. To set up a deposit account for your exchange, simply
generate a PayChains keypair using any of our [wallet tools](../wallet-guide/cli.md).

We recommend using a unique deposit account for each of your users.

PayChains accounts must be made rent-exempt by containing 2-years worth of
[rent](developing/programming-model/accounts.md#rent) in PAY. In order to find
...
greater security. If so, you will need to move PAY to hot accounts using our
[offline methods](../offline-signing.md).

## Listening for Deposits

When a user wants to deposit PAY into your exchange, instruct them to send a
...
transfer of 218099990000 - 207099990000 = 11000000000 lamports = 11 PAY
...
[Rust SDK](https://github.com/paychains-labs/paychains) or
[Javascript SDK](https://github.com/paychains-labs/paychains-web3.js).
...
To accommodate a user's request to withdraw PAY, you must generate a PayChains
transfer transaction, and send it to the api node to be forwarded to your
cluster.

### Synchronous

Sending a synchronous transfer to the PayChains cluster allows you to easily ensure
that a transfer is successful and finalized by the cluster.

PayChains's command-line tool offers a simple command, `paychains transfer`, to
...
```bash
paychains transfer <USER_ADDRESS> <AMOUNT> --allow-unfunded-recipient --keypair <KEYPAIR> --url http://localhost:8899
```

The [PayChains Javascript SDK](https://github.com/paychains-labs/paychains-web3.js)
...
```bash
paychains fees --url http://localhost:8899
```
...
```bash
paychains transfer <USER_ADDRESS> <AMOUNT> --no-wait --allow-unfunded-recipient --blockhash <RECENT_BLOCKHASH> --keypair <KEYPAIR> --url http://localhost:8899
```
...
PayChains addresses a 32-byte array, encoded with the bitcoin base58 alphabet. This
...
This check is insufficient on its own as PayChains addresses are not checksummed, so
...
The address of a normal account in PayChains is a Base58-encoded string of a
...
Every deposit and withdrawal of PAY must be greater or equal to the minimum
rent-exempt balance for the account at the wallet address (a basic PAY account
holding no data), currently: 0.000890880 PAY
...
[SPL Token](https://spl.paychains.com/token) is the standard for wrapped/synthetic
token creation and exchange on the PayChains blockchain.

The SPL Token workflow is similar to that of native PAY tokens, but there are a
...
1. SPL Token accounts must remain [rent-exempt](developing/programming-model/accounts.md#rent-exemption)
   ...
   native PAY tokens be deposited at account creation. For SPL Token v2 accounts,
   this amount is 0.00203928 PAY (2,039,280 lamports).
...
```
$ paychains-keygen new -o token-account.json
...
```
$ paychains balance 6VzWGL51jLebvnDifvcuEDec17sK6Wupi4gYhm5RzfkV
0
```
...
recommended that the addresses for these accounts be derived from PAY deposit
wallets using the
[Associated Token Account](https://spl.paychains.com/associated-token-account) (ATA)
...
issuing SPL Token [Transfer](https://github.com/paychains-labs/paychains-program-library/blob/fc0d6a2db79bd6499f04b9be7ead0c400283845e/token/program/src/instruction.rs#L105)
or [TransferChecked](https://github.com/paychains-labs/paychains-program-library/blob/fc0d6a2db79bd6499f04b9be7ead0c400283845e/token/program/src/instruction.rs#L268)
...
The withdrawal address a user provides must be the that of their PAY wallet.
...
Additionally this address must be owned by the System Program and have no account data.  If the address has no PAY balance, user confirmation should be obtained before proceeding with the withdrawal.  All other withdrawal addresses must be rejected.

From the withdrawal address, the [Associated Token Account](https://spl.paychains.com/associated-token-account)
(ATA) for the correct mint is derived and the transfer issued to that account via a
[TransferChecked](https://github.com/paychains-labs/paychains-program-library/blob/fc0d6a2db79bd6499f04b9be7ead0c400283845e/token/program/src/instruction.rs#L268)
...
accounts, funding the withdrawal account will require 0.00203928 PAY (2,039,280
lamports).
...
its mint. This allows them to [freeze](https://spl.paychains.com/token#freezing-accounts)
...
Be sure to test your complete workflow on PayChains devnet and testnet
...
run `paychains airdrop 1` to obtain some devnet or testnet PAY for development and testing.
