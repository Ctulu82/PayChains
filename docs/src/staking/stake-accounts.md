...
A stake account on PayChains can be used to delegate tokens to validators on
...
receive PAY from other accounts on the network, whereas a stake account supports
more complex operations needed to manage a delegation of tokens.

Stake accounts on PayChains also work differently than those of other Proof-of-Stake
blockchain networks that you may be familiar with. This document describes the
high-level structure and functions of a PayChains stake account.
...
Like other types of accounts on the PayChains network, a stake account that has a
balance of 0 PAY is no longer tracked. If a stake account is not delegated
...
Stake account details can be viewed on the [PayChains Explorer](http://explorer.paychains.com/accounts)
