...
```bash
paychains catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your PayChains
runtime directory.

```bash
paychains-keygen new -o ~/validator-stake-keypair.json
```

## Delegate Stake

Now delegate 1 PAY to your validator by first creating your stake account:

```bash
paychains create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
paychains delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```

> Don’t delegate your remaining PAY, as your validator will use those tokens to vote.
...
```bash
paychains delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```
...
- View your vote account:`paychains vote-account ~/vote-account-keypair.json` This displays the current state of all the votes the validator has submitted to the network.
- View your stake account, the delegation preference and details of your stake:`paychains stake-account ~/validator-stake-keypair.json`
- `paychains validators` displays the current active stake of all validators, including yours
- `paychains stake-history` shows the history of stake warming up and cooling down over recent epochs
- Look for log messages on your validator indicating your next leader slot: `[2019-09-27T20:16:00.319721164Z INFO paychains_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your validator by running `paychains validators`

## Monitor Your Staked Validator

Confirm your validator becomes a [leader](../terminology.md#leader)

- After your validator is caught up, use the `paychains balance` command to monitor the earnings as your validator is selected as leader and collects transaction fees
- PayChains nodes offer a number of useful JSON-RPC methods to return information about the network and your validator's participation. Make a request by using curl \(or another http client of your choosing\), specifying the desired method in JSON-RPC-formatted data. For example:
...
- `getVoteAccounts` This will tell you how much active stake your validator currently has. A % of the validator's stake is activated on an epoch boundary. You can learn more about staking on PayChains [here](../cluster/stake-delegation-and-rewards.md).
...
```bash
paychains deactivate-stake ~/validator-stake-keypair.json
```
