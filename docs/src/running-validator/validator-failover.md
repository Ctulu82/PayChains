...
### Primary Validator
The following additional `paychains-validator` parameters are required to enable
...
following `paychains-validator` command-line argument changes:
...
```bash
$ paychains-validator wait-for-restart-window --identity validator-keypair.json \
  && paychains-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `paychains-validator set-identity
