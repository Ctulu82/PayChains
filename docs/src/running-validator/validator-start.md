...
## Configure PayChains CLI

The paychains cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
paychains config set --url http://api.devnet.paychains.com
```

While this section demonstrates how to connect to the Devnet cluster, the steps
are similar for the other [PayChains Clusters](../clusters.md).
...
```bash
paychains transaction-count
```

View the [metrics dashboard](https://metrics.paychains.com:3000/d/monitor/cluster-telemetry) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `paychains-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> paychains::validator] CUDA is enabled"`
...
The paychains repo includes a daemon to adjust system settings to optimize performance
(namely by increasing the OS UDP buffer and file mapping limits).

The daemon (`paychains-sys-tuner`) is included in the paychains binary release. Restart
...
```bash
sudo $(command -v paychains-sys-tuner) --user $(whoami) > sys-tuner.log 2>&1 &
```
...
```bash
sudo bash -c "cat >/etc/sysctl.d/20-paychains-udp-buffers.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/20-paychains-udp-buffers.conf
```

##### **Increased memory mapped files limit**

```bash
sudo bash -c "cat >/etc/sysctl.d/20-paychains-mmaps.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/20-paychains-mmaps.conf
```
...
```bash
sudo bash -c "cat >/etc/security/limits.d/90-paychains-nofiles.conf <<EOF
...
```bash
paychains-keygen new -o ~/validator-keypair.json
```

The identity public key can now be viewed by running:

```bash
paychains-keygen pubkey ~/validator-keypair.json
```
...
You can generate a custom vanity keypair using paychains-keygen. For instance:

```bash
paychains-keygen grind --starts-with e1v1s:1
```
...
```bash
paychains-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
## More PayChains CLI Configuration

Now that you have a keypair, set the paychains configuration to use your validator
keypair for all following commands:

```bash
paychains config set --keypair ~/validator-keypair.json
```

You should see the following output:

```text
Config File: /home/paychains/.config/paychains/cli/config.yml
RPC URL: http://api.devnet.paychains.com
WebSocket URL: ws://api.devnet.paychains.com/ (computed)
Keypair Path: /home/paychains/validator-keypair.json
Commitment: confirmed
```

## Airdrop & Check Validator Balance

Airdrop yourself some PAY to get started:
...
```text
paychains balance --lamports
```

Read more about the [difference between PAY and lamports here](../introduction.md#what-are-sols).
...
```bash
paychains-keygen new -o ~/authorized-withdrawer-keypair.json
```
...
“vote-account-keypair.json” in your PayChains runtime directory:

```bash
paychains-keygen new -o ~/vote-account-keypair.json
```
...
```bash
paychains create-vote-account ~/vote-account-keypair.json ~/validator-keypair.json ~/authorized-withdrawer-keypair.json
```
...
argument to `paychains-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
> `paychains-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `paychains-validator --dynamic-port-range 11000-11020 ...` will restrict
...
if desired. Check `paychains-validator --help` for the default limit value used by
...
```
[Unit]
Description=PayChains Validator
After=network.target
Wants=paychains-sys-tuner.service
...
Environment="PATH=/bin:/usr/bin:/home/sol/.local/share/paychains/install/active_release/bin"
...
`paychains-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec paychains-validator ...").  This is
...
The validator log file, as specified by `--log ~/paychains-validator.log`, can get
...
launch the process with `exec` (`exec paychains-validator ...`) when using logrotate.
...
/home/sol/paychains-validator.log:
...
which starts the paychains validator process uses "exec" to do so (example: "exec
paychains-validator ..."); otherwise, when logrotate sends its signal to the
...
`paychains-validator` command-line.
...
1. `sudo mkdir /mnt/paychains-accounts`
2. Add a 300GB tmpfs parition by adding a new line containing `tmpfs /mnt/paychains-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab`
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/paychains-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/paychains-accounts` argument to your `paychains-validator`
