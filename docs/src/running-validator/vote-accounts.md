...
account is needed if you plan to run a validator node on PayChains.

## Create a Vote Account

A vote account can be created with the
[create-vote-account](../cli/usage.md#paychains-create-vote-account) command.
...
- To change the [validator identity](#validator-identity), use
  [vote-update-validator](../cli/usage.md#paychains-vote-update-validator).
- To change the [vote authority](#vote-authority), use
  [vote-authorize-voter-checked](../cli/usage.md#paychains-vote-authorize-voter-checked).
- To change the [authorized withdrawer](#authorized-withdrawer), use
  [vote-authorize-withdrawer-checked](../cli/usage.md#paychains-vote-authorize-withdrawer-checked).
- To change the [commission](#commission), use
  [vote-update-commission](../cli/usage.md#paychains-vote-update-commission).
...
the validator operator may choose to store only enough PAY on the identity
...
the [vote-update-validator](../cli/usage.md#paychains-vote-update-validator) command.
...
the transaction fee. Because transaction fees on PayChains are assessed
...
[vote-authorize-voter-checked](../cli/usage.md#paychains-vote-authorize-voter-checked) command.

The vote authority can be changed at most once per epoch. If the authority is
changed with [vote-authorize-voter-checked](../cli/usage.md#paychains-vote-authorize-voter-checked),
this will not take effect until the beginning of the next epoch.
To support a smooth transition of the vote signing,
`paychains-validator` allows the `--authorized-voter` argument to be specified
...
using the [withdraw-from-vote-account](../cli/usage.md#paychains-withdraw-from-vote-account)
...
[vote-update-commission](../cli/usage.md#paychains-vote-update-commission) command.
...
1. Create the new validator identity keypair, `paychains-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded, `paychains transfer ~/new-validator-keypair.json 500`.
3. Run `paychains vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `paychains leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `paychains-validator` for a seamless
migration.

1. Run `paychains epoch-info`. If there is not much time remaining time in the
   current epoch, consider waiting for the next epoch to allow your validator
   plenty of time to restart and catch up.
2. Create the new vote authority keypair, `paychains-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running `paychains vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run `paychains vote-authorize-voter ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next epoch.
5. `paychains-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `paychains-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling is required. Use the `paychains vote-authorize-withdrawer` command as needed.

## Close a Vote Account

A vote account can be closed with the
[close-vote-account](../cli/usage.md#paychains-close-vote-account) command.
Closing a vote account withdraws all remaining PAY funds to a supplied recipient address and renders it invalid as a vote account.
