...
Messages transmitted to PayChains validators must not exceed the IPv6 MTU size to
ensure fast and reliable network transmission of cluster info over UDP.
PayChains's networking stack uses a conservative MTU size of 1280 bytes which,
after accounting for headers, leaves 1232 bytes for packet data like serialized
transactions.

Developers building applications on PayChains must design their on-chain program
...
affect transaction processing in the PayChains program runtime beyond the
...
because it adds an extra step for transaction processing. Instead, PayChains
...
PayChains can provide a new on-chain program which allows "Big" transactions to be
