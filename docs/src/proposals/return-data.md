...
The callee can set the return data using a new system call `pay_set_return_data(buf: *const u8, length: u64)`.
...
The return data can be retrieved with `pay_get_return_data(buf: *mut u8, length: u64, program_id: *mut Pubkey) -> u64`.
...
When an instruction calls `pay_invoke()`, the return data of the callee is copied into the return data
...
Note that `pay_invoke()` clears the returns data before invoking the callee, so that any return data from
...
For a normal RPC or Transaction, the returndata is base64-encoded and stored along side the pay_log
strings in the [stable log](https://github.com/paychains-labs/paychains/blob/95292841947763bdd47ef116b40fc34d0585bca8/sdk/src/process_instruction.rs#L275-L281).

## Note on returning errors

Solidity on Ethereum allows the contract to return an error in the return data. In this case, all
the account data changes for the account should be reverted. On PayChains, any non-zero exit code
...
Errors will be reported via pay_log.
