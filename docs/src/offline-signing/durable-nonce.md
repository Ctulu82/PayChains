...
They are implemented as a PayChains Program, the mechanics of which can be read
...
> [Full usage documentation](../cli/usage.md#paychains-create-nonce-account)
...
```bash
paychains nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#paychains-get-nonce)
...
```bash
paychains new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#paychains-new-nonce)
...
```bash
paychains nonce-account nonce-keypair.json
```
...
```text
balance: 0.5 PAY
minimum balance required: 0.00136416 PAY
nonce: DZar6t2EaCFQTbUP4DHKwZ1wT8gCPW2aRfkVWhydkBvS
```

> [Full usage documentation](../cli/usage.md#paychains-nonce-account)
...
```bash
paychains withdraw-from-nonce-account nonce-keypair.json ~/.config/paychains/id.json 0.5
```
...
> [Full usage documentation](../cli/usage.md#paychains-withdraw-from-nonce-account)
...
```bash
paychains authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../cli/usage.md#paychains-authorize-nonce-account)
...
- [`pay`](../cli/usage.md#paychains-pay)
...
Here we demonstrate Alice paying Bob 1 PAY using a durable nonce. The procedure
...
```bash
$ paychains-keygen new -o alice.json
$ paychains-keygen new -o nonce.json
$ paychains-keygen new -o bob.json
```
...
```bash
$ paychains airdrop -k alice.json 1
1 PAY
```
...
```bash
$ paychains create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ paychains pay -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR paychains_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ paychains nonce-account nonce.json
balance: 0.1 PAY
minimum balance required: 0.00136416 PAY
...
```bash
$ paychains pay -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
HR1368UKHVZyenmH7yVz5sBAijV6XAPeWbEiXEGVYQorRMcoijeNAbzZqEZiH8cDB8tk65ckqeegFjK8dHwNFgQ
```

#### - Success!

The transaction succeeds! Bob receives 0.01 PAY from Alice and Alice's stored
nonce advances to a new value

```bash
$ paychains balance -k bob.json
0.01 PAY
```
