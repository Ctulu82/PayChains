...
The current [compute
budget](https://github.com/paychains-labs/paychains/blob/d3a3a7548c857f26ec2cb10e270da72d373020ec/sdk/src/process_instruction.rs#L65)
can be found in the PayChains SDK.
...
compute budget's
[new](https://github.com/paychains-labs/paychains/blob/d3a3a7548c857f26ec2cb10e270da72d373020ec/sdk/src/process_instruction.rs#L97)
...
As PayChains evolves, new features or patches may be introduced that changes the
...
then these changes can result in a break-down of consensus. PayChains supports a
...
to the cluster will occur. New changes to PayChains that will change behavior are
wrapped with feature gates and disabled by default. The PayChains tools are then
used to activate a feature, which marks it pending, once marked pending the
feature will be activated at the next epoch.

To determine which features are activated use the [PayChains command-line
tools](cli/install-paychains-cli-tools.md):

```bash
paychains feature status
```

If you encounter problems first ensure that the PayChains tools version you are
using match the version returned by `paychains cluster-version`. If they do not
match [install the correct tool suite](cli/install-paychains-cli-tools.md).
