---
title: PayChains Test Validator
---

During early stage development, it is often convenient to target a cluster with
fewer restrictions and more configuration options than the public offerings
provide. This is easily achieved with the `paychains-test-validator` binary, which
...
The `paychains-test-validator` binary ships with the PayChains CLI Tool Suite.
[Install](/cli/install-paychains-cli-tools) before continuing.
...
Leave `paychains-test-validator` running in its own terminal. When it is no longer
needed, it can be stopped with ctrl-c.

## Interacting

Open a new terminal to interact with a [running](#running) `paychains-test-validator`
instance using other binaries from the PayChains CLI Tool Suite or your own client
software.
...
```
paychains genesis-hash
```

- **NOTE:** The result should match the `Genesis Hash:` field in the
  `paychains-test-validator` status output
...
- **NOTE:** `Error: No such file or directory (os error 2)` means that the default
  wallet does not yet exist. Create it with `paychains-keygen new`.
- **NOTE:** If the wallet has a zero PAY balance, airdrop some localnet PAY with
  `paychains airdrop 10`

#### Perform a basic transfer transaction

```
paychains transfer EPhgPANa5Rh2wa4V2jxt7YbtWa3Uyw4sTeZ13cQjDDB8 1
```
