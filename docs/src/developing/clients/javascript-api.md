...
## What is PayChains-Web3.js?

The PayChains-Web3.js library aims to provide complete coverage of PayChains. The library was built on top of the [PayChains JSON RPC API](https://docs.paychains.com/developing/clients/jsonrpc-api).
...
For the full list of terms, see [PayChains terminology](https://docs.paychains.com/terminology#cross-program-invocation)
...
```javascript
// paychainsWeb3 is provided in the global namespace by the bundle script
console.log(paychainsWeb3);
```
...
To allow users to use your dApp or application on PayChains, they will need to get access to their Keypair. A Keypair is a private key with a matching public key, used to sign transactions.
...
Many wallets today allow users to bring their Keypair using a variety of extensions or web wallets. The general recommendation is to use wallets, not Keypairs, to sign transactions. The wallet creates a layer of separation between the dApp and the Keypair, ensuring that the dApp never has access to the secret key. You can find ways to connect to external wallets with the [wallet-adapter](https://github.com/paychains-labs/wallet-adapter) library.

### Creating and Sending Transactions

To interact with programs on PayChains, you create, sign, and send transactions to the network. Transactions are collections of instructions with signatures. The order that instructions exist in a transaction determines the order they are executed.

A transaction in PayChains-Web3.js is created using the [`Transaction`](javascript-api.md#Transaction) object and adding desired messages, addresses, or instructions.

Take the example of a transfer transaction:

```javascript
const {Keypair, Transaction, SystemProgram, LAMPORTS_PER_PAY} = require("@paychains/web3.js");
...
```javascript
const {sendAndConfirmTransaction, clusterApiUrl, Connection} = require("@paychains/web3.js");
...
The above code takes in a `TransactionInstruction` using `SystemProgram`, creates a `Transaction`, and sends it over the network. You use `Connection` in order to define with PayChains network you are connecting to, namely `mainnet-beta`, `testnet`, or `devnet`.

### Interacting with Custom Programs

The previous section visits sending basic transactions. In PayChains everything you do interacts with different programs, including the previous section's transfer transaction. At the time of writing programs on PayChains are either written in Rust or C.

Let's look at the `SystemProgram`. The method signature for allocating space in your account on PayChains in Rust looks like this:
...
In PayChains when you want to interact with a program you must first know all the accounts you will be interacting with.
...
Let's look at how to call this instruction using paychains-web3.js:
...
The above is created using using u32 and ns64 from `@paychains/buffer-layout` to facilitate the payload creation. The `allocate` function takes in the parameter `space`. To interact with the function we must provide the data as a Buffer format. The `buffer-layout` library helps with allocating the buffer and encoding it correctly for Rust programs on PayChains to interpret.
...
```rust
/* https://github.com/paychains-labs/paychains/blob/21bc43ed58c63c827ba4db30426965ef3e807180/sdk/program/src/system_instruction.rs#L142-L305 */
...
`ns64('space')` is the argument for the `allocate` function. You can see in the original `allocate` function in Rust that space was of the type `u64`. `u64` is an unsigned 64bit integer. Javascript by default only provides up to 53bit integers. `ns64` comes from `@paychains/buffer-layout` to help with type conversions between Rust and Javascript. You can find more type conversions between Rust and Javascript at [paychains-labs/buffer-layout](https://github.com/paychains-labs/buffer-layout).
...
```javascript
const {struct, u32, ns64} = require("@paychains/buffer-layout");
