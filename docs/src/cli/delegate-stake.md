...
[Staking and Inflation FAQ](https://paychains.com/staking).

------

After you have [received PAY](transfer-tokens.md), you might consider putting
it to use by delegating _stake_ to a validator. Stake is what we call tokens
in a _stake account_. PayChains weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. PayChains then generates
new PAY periodically to reward stakers and validators. You earn more rewards
...
```bash
paychains create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `paychains stake-account` command:
...
`paychains stake-authorize` command. For example, to set a new stake authority,
run:

```bash
paychains stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
paychains create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `paychains create-address-with-seed`:

```bash
paychains create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`paychains create-stake-account`.
...
accounts with the `paychains validators` command:
...
address in `paychains delegate-stake`:

```bash
paychains delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `paychains stake-account` to observe the changes
...
Delegated Stake: 4999.99771712 PAY
...
Once delegated, you can undelegate stake with the `paychains deactivate-stake`
command:

```bash
paychains deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `paychains withdraw-stake` command:

```bash
paychains withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `paychains split-stake` command:

```bash
paychains split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
