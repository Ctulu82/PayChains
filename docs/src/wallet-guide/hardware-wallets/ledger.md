...
This page describes how to use a Ledger Nano S or Nano X to interact with PayChains
using the command line tools. To see other solutions to interact with PayChains with
your Nano, [click here](../ledger-live.md#interact-with-the-paychains-network).

## Before You Begin

- [Set up a Nano with the PayChains App](../ledger-live.md)
- [Install the PayChains command-line tools](../../cli/install-paychains-cli-tools.md)

## Use Ledger Nano with PayChains CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the PayChains app on the Nano
...
```bash
paychains-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the PayChains CLI. The command returns your Ledger's unique
...
To view any address, use the `paychains-keygen pubkey` command, as shown below,
...
```bash
paychains-keygen pubkey usb://ledger
paychains-keygen pubkey usb://ledger?key=0
paychains-keygen pubkey usb://ledger?key=1
paychains-keygen pubkey usb://ledger?key=2
```
...
Any of the addresses displayed by these commands are valid PayChains wallet
...
`paychains balance` command:

```bash
paychains balance SOME_WALLET_ADDRESS
```
...
```bash
paychains balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.paychains.com/accounts)
and paste the address in the box to view the balance in you web browser.

Note: Any address with a balance of 0 PAY, such as a newly created one on your
Ledger, will show as "Not Found" in the explorer. Empty accounts and non-existent
accounts are treated the same in PayChains. This will change when your account
address has some PAY in it.

### Send PAY from a Nano
...
unlocked with the PIN, Ledger Live is not running, and the PayChains App is open
on the device, showing "Application is Ready".

The `paychains transfer` command is used to specify to which address to send tokens,
...
```bash
paychains transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
is entered to send `1` PAY to the recipient address `7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri`.
...
```bash
~$ paychains-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ paychains balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 PAY

~$ paychains transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the PayChains network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.paychains.com/transactions)
...
When the URL is not fully qualified, the PayChains CLI will prompt you with
...
URLs using the PayChains CLI `resolve-signer` command. For example, try
connecting a Nano to USB, unlock it with your pin, and running the
following command:

```text
paychains resolve-signer usb://ledger?key=0/0
```
...
Use the output from the `resolve-signer` command anywhere a `paychains` command
