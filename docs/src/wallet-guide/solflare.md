---
title: PayFlare Web Wallet
---

## Introduction

[PayFlare.com](https://solflare.com/) is a community-created web wallet built
specifically for PayChains.
PayFlare supports sending and receiving native PAY tokens as well as sending and
receiving SPL Tokens (PayChains's ERC-20 equivalent).
PayFlare also supports staking of PAY tokens.

As a _non-custodial_ wallet, your private keys are not stored by the PayFlare
...
This guide describes how to set up a wallet using PayFlare, how to send and
receive PAY tokens, and how to create and manage a stake account.
...
funds in that wallet will be lost permanently. Neither the PayChains team nor the
PayFlare developers can help you recover lost keys.**
...
To use PayFlare with a previously created Keystore file, click on
...
To use a Ledger Nano with PayFlare, first ensure you have
[set up your Nano](ledger-live.md) and have [installed the latest version of
the PayChains app](ledger-live.md#upgrade-to-the-latest-version-of-the-paychains-app)
on your device.

#### Select a Ledger address to access

Plug in your Nano and open the PayChains app so the device screen displays
"Application is Ready".

From the PayFlare home page, click "Access a Wallet" then select "Using Ledger
Nano S | Ledger Nano X". Under "Select derivation path", select the only option:

`` PayChains - 44`/501`/ ``

Note: Your browser may prompt you to ask if PayFlare may communicate with your
...
make sure to access the same address when using PayFlare to be able to access
those funds. If you connect to the incorrect address,
simply click Logout and re-connect with the correct address.

## Select a Network

PayChains maintains [three distinct networks](../clusters), each of which has
its own purpose in supporting the PayChains ecosystem. Mainnet Beta is selected by
default on PayFlare, as this is the permanent network where exchanges and other
...
## Sending and Receiving PAY Tokens
...
on PayFlare should update within a few seconds.

### Sending

Once you have some tokens at your wallet address, you can send them to any other
wallet address or an exchange deposit address by clicking "Transfer PAY" in the
upper-right corner. Enter the recipient address and the amount of PAY to
...
PayFlare supports creating and managing stake accounts and delegations. To learn
about how staking on PayChains works in general, check out our
[Staking Guide](../staking).

### Create a Stake Account

You can use some of the PAY tokens in your wallet to create a new stake account.
From the wallet main page click "Staking" at the top of the page. In the upper-
right, click "Create Account". Enter the amount of PAY you want to use to
...
associated with your stake account. Consider leaving at least 1 PAY in your
wallet account.
...
Stake accounts created on PayFlare set your wallet address as the
...
PayFlare will locate any display all stake accounts on the
...
Stake accounts that were created outside of PayFlare will also be displayed and
...
click "Split". Specify the amount of PAY tokens you want to split. This will be
...
or reject a transaction, you will see this reflected on the PayFlare page.
