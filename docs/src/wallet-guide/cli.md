...
PayChains supports several different types of wallets that can be used to interface
directly with the PayChains command-line tools.

**If you are unfamiliar with using command line programs and just want to be able
to send and receive PAY tokens, we recommend setting up a third-party
[App Wallet](apps.md)**.

To use a Command Line Wallet, you must first [install the PayChains CLI tools](../cli/install-paychains-cli-tools.md)
