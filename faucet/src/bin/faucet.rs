use {
    ...
    log::*,
    paychains_clap_utils::input_parsers::{lamports_of_pay, value_of},
    paychains_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    paychains_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = paychains_cli_config::Config::default().keypair_path;

    paychains_logger::setup_with_default("paychains=info");
    paychains_metrics::set_panic_hook("faucet");
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in PAY"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in PAY"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_pay(&matches, "per_time_cap");
    let per_request_cap = lamports_of_pay(&matches, "per_request_cap");
