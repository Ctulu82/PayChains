use {
    ...
    log::*,
    rayon::prelude::*,
    paychains_client::perf_utils::{sample_txs, SampleStats},
    paychains_core::gen_keys::GenKeys,
    paychains_faucet::faucet::request_airdrop_transaction,
    paychains_measure::measure::Measure,
    paychains_metrics::{self, datapoint_info},
    paychains_sdk::{
        client::Client,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    Builder::new()
        .name("paychains-client-sample".to_string())
        .spawn(move || {
    ...
    T: 'static + Client + Send + Sync,
{
    (0..threads)
        .map(|_| {
            ...
            let client = client.clone();
            Builder::new()
                .name("paychains-client-sender".to_string())
                .spawn(move || {
                    do_tx_transfers(
                        &exit_signal,
                        &shared_txs,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    let blockhash_thread = {
        ...
        let id = id.pubkey();
        Builder::new()
            .name("paychains-blockhash-poller".to_string())
            .spawn(move || {
...
pub fn generate_and_fund_keypairs<T: 'static + Client + Send + Sync>(
    ...
    // Sample the first keypair, to prevent lamport loss on repeated paychains-bench-tps executions
