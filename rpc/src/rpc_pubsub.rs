...
use {
    ...
    paychains_transaction_status::UiTransactionEncoding,
    std::{str::FromStr, sync::Arc},
};

// We have to keep both of the following traits to not break backwards compatibility.
// `RpcPayPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcPayPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcPayPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcPayPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcPayPubSubInternal for RpcPayPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        paychains_account_decoder::{parse_account_data::parse_account_data, UiAccountEncoding},
        paychains_client::rpc_response::{
        ...
        paychains_vote_program::vote_state::{Vote, VoteTransaction},
        std::{
            sync::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = paychains_sdk::pubkey::new_rand();
