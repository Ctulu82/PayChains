...
rm -rf "$PAYCHAINS_CONFIG_DIR"/latest-mainnet-beta-snapshot
mkdir -p "$PAYCHAINS_CONFIG_DIR"/latest-mainnet-beta-snapshot
(
  cd "$PAYCHAINS_CONFIG_DIR"/latest-mainnet-beta-snapshot || exit 1
  set -x
  wget http://api.mainnet-beta.paychains.com/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.paychains.com/snapshot.tar.bz2
)

snapshot=$(ls "$PAYCHAINS_CONFIG_DIR"/latest-mainnet-beta-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$paychains_ledger_tool create-snapshot \
  --ledger "$PAYCHAINS_CONFIG_DIR"/latest-mainnet-beta-snapshot \
