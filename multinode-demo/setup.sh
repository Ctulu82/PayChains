...
if [[ -f $BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR" "$PAYCHAINS_CONFIG_DIR"/bootstrap-validator/stake-account.json
else
...
if [[ -f $BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR" "$PAYCHAINS_CONFIG_DIR"/bootstrap-validator/vote-account.json
else
...
"$PAYCHAINS_ROOT"/fetch-spl.sh
if [[ -r spl-genesis-args.sh ]]; then
  SPL_GENESIS_ARGS=$(cat "$PAYCHAINS_ROOT"/spl-genesis-args.sh)
  #shellcheck disable=SC2207
  #shellcheck disable=SC2206
  args+=($SPL_GENESIS_ARGS)
fi

default_arg --ledger "$PAYCHAINS_CONFIG_DIR"/bootstrap-validator
default_arg --faucet-pubkey "$PAYCHAINS_CONFIG_DIR"/faucet.json
default_arg --faucet-lamports 500000000000000000
default_arg --hashes-per-tick auto
default_arg --cluster-type development

$paychains_genesis "${args[@]}"
