...
airdrops_enabled=1
node_pay=500 # 500 PAY: number of PAY to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-pay PAY            - Number of PAY this node has been funded from the genesis config (default: $node_pay)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-pay ]]; then
      node_pay="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # paychains-validator options
...
if [[ "$PAYCHAINS_GPU_MISSING" -eq 1 ]]; then
...
if [[ -n $REQUIRE_LEDGER_DIR ]]; then
  ...
  PAYCHAINS_CONFIG_DIR="$ledger_dir"
fi
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$PAYCHAINS_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $PAYCHAINS_CUDA ]]; then
  program=$paychains_validator_cuda
else
  program=$paychains_validator
fi
...
wallet() {
  (
    set -x
    $paychains_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
setup_validator_accounts() {
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      echo "Adding $node_pay to validator identity account:"
      (
        set -x
        $paychains_cli \
          --keypair "$PAYCHAINS_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_pay"
      ) || return $?
    fi
...
rpc_url=$($paychains_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $paychains_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $paychains_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $paychains_keygen new --no-passphrase -so "$authorized_withdrawer"

setup_validator_accounts "$node_pay"
