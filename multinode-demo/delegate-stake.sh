...
stake_pay=1   # default number of PAY to assign as stake (1 PAY)
...
usage: $0 [OPTIONS] <PAY to stake ($stake_pay)>
...
if [[ -n ${positional_args[0]} ]]; then
  stake_pay=${positional_args[0]}
fi

VALIDATOR_KEYS_DIR=$PAYCHAINS_CONFIG_DIR/validator$label
...
if ((airdrops_enabled)); then
  ...
  $paychains_cli \
    "${common_args[@]}" --keypair "$PAYCHAINS_CONFIG_DIR/faucet.json" \
    transfer --allow-unfunded-recipient "$keypair" "$stake_pay"
fi
...
if ! [[ -f "$stake_account" ]]; then
  $paychains_keygen new --no-passphrase -so "$stake_account"
else
  echo "$stake_account already exists! Using it"
fi

set -x
$paychains_cli "${common_args[@]}" \
  vote-account "$vote_account"
$paychains_cli "${common_args[@]}" \
  create-stake-account "$stake_account" "$stake_pay"
$paychains_cli "${common_args[@]}" \
  delegate-stake $maybe_force "$stake_account" "$vote_account"
$paychains_cli "${common_args[@]}" stakes "$stake_account"
