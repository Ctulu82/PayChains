...
function analyze_packet_loss {
  (
    ...
    for ip in "${validatorIpList[@]}"; do
      "${REPO_ROOT}"/net/scp.sh ip_address_map.txt paychains@"$ip":~/paychains/
    done
    ...
    for ip in "${validatorIpList[@]}"; do
      ...
      "${REPO_ROOT}"/net/ssh.sh paychains@"$ip" 'PATH=$PATH:~/.cargo/bin/ ~/paychains/scripts/iftop-postprocess.sh ~/paychains/iftop.log temp.log ~paychains/paychains/ip_address_map.txt' > "$iftop_log"
      upload-ci-artifact "$iftop_log"
    done

    execution_step "Analyzing Packet Loss"
    "${REPO_ROOT}"/paychains-release/bin/paychains-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_max_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/paychains wait-for-max-stake $max_stake --url http://127.0.0.1:8899"
}
...
function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/paychains --url http://127.0.0.1:8899 slot'
}
...
function get_active_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/paychains --url http://127.0.0.1:8899 validators --output=json | grep -o "totalActiveStake\": [0-9]*" | cut -d: -f2'
}
...
function get_current_stake {
  ...
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/paychains --url http://127.0.0.1:8899 validators --output=json | grep -o "totalCurrentStake\": [0-9]*" | cut -d: -f2'
}
...
function upload_results_to_slack() {
  ...
  COMMIT_URL="https://github.com/paychains-labs/paychains/commit/${COMMIT}"

  if [[ -n $BUILDKITE_BUILD_URL ]] ; then
    BUILD_BUTTON_TEXT="Build Kite Job"
  else
    BUILD_BUTTON_TEXT="Build URL not defined"
    BUILDKITE_BUILD_URL="https://buildkite.com/paychains-labs/"
  fi

  GRAFANA_URL="https://internal-metrics.paychains.com:3000/d/monitor-${CHANNEL:-edge}/cluster-telemetry-${CHANNEL:-edge}?var-testnet=${TESTNET_TAG:-testnet-automation}&from=${TESTNET_START_UNIX_MSECS:-0}&to=${TESTNET_FINISH_UNIX_MSECS:-0}"
