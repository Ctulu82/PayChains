...
function parse_stake_account_data_to_file {
  ...
  account_data="$(paychains --url "$url" show-stake-account "$account_key")"
...
function display_results_summary {
  ...
  stake_account_balance_total_pay="$(bc <<< "scale=3; $stake_account_balance_total/$LAMPORTS_PER_PAY")"
  system_account_balance_pay="$(bc <<< "scale=3; $system_account_balance/$LAMPORTS_PER_PAY")"

  all_account_total_balance="$(bc <<< "scale=3; $system_account_balance+$stake_account_balance_total")"
  all_account_total_balance_pay="$(bc <<< "scale=3; ($system_account_balance+$stake_account_balance_total)/$LAMPORTS_PER_PAY")"
  ...
  printf "Balance of all STAKE accounts: %'.3f PAY\n" "$stake_account_balance_total_pay"
  printf "\n"
  printf "Balance of SYSTEM account: %'d lamports\n" "$system_account_balance"
  printf "Balance of SYSTEM account: %'.3f PAY\n" "$system_account_balance_pay"
  printf "\n"
  printf "Total Balance of ALL accounts: %'d lamports\n" "$all_account_total_balance"
  printf "Total Balance of ALL accounts: %'.3f PAY\n" "$all_account_total_balance_pay"
  echo "--------------------------------------------------------------------------------------"
}
