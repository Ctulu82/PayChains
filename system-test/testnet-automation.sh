...
#     git clone https://github.com/paychains-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
    git clone git@github.com:paychains-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "paychains-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://internal-metrics.paychains.com:8086
...
if [[ -z $PAYCHAINS_METRICS_CONFIG ]]; then
  if [[ -z $PAYCHAINS_METRICS_PARTIAL_CONFIG ]]; then
    echo PAYCHAINS_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export PAYCHAINS_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$PAYCHAINS_METRICS_PARTIAL_CONFIG"
fi
echo "PAYCHAINS_METRICS_CONFIG: $PAYCHAINS_METRICS_CONFIG"
