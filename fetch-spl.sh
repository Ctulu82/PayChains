#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the paychains-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/paychains-spl/$so ]]; then
    cp ~/.cache/paychains-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/paychains-labs/paychains-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/paychains-spl
    cp "$so" ~/.cache/paychains-spl/"$so"
  fi
...
echo
echo "paychains-genesis command-line arguments (spl-genesis-args.sh):"
