...
use {
    ...
    paychains_perf::packet::{Packet, PacketBatch},
    paychains_rayon_threadlimit::get_thread_count,
    paychains_runtime::{bank::Bank, bank_forks::BankForks},
    paychains_sdk::{clock::Slot, packet::PACKET_DATA_SIZE, pubkey::Pubkey},
    std::{
        cmp::Reverse,
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("paychains-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("paychains-check-duplicate".to_string())
            .spawn(move || loop {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("paychains-window-insert-error", 1, 1);
        };

        Builder::new()
            .name("paychains-window-insert".to_string())
            .spawn(move || {
            ...
            + std::marker::Sync,
    {
        ...
        Builder::new()
            .name("paychains-window".to_string())
            .spawn(move || {
                ...
                let handle_error = || {
                    inc_new_counter_error!("paychains-window-error", 1, 1);
                };
...
mod test {
    ...
    fn test_should_retransmit_and_persist() {
        let me_id = paychains_sdk::pubkey::new_rand();
