...
impl Default for MetricsAgent {
    fn default() -> Self {
        let max_points_per_sec = env::var("PAYCHAINS_METRICS_MAX_POINTS_PER_SECOND")
            .map(|x| {
                x.parse()
                    .expect("Failed to parse PAYCHAINS_METRICS_MAX_POINTS_PER_SECOND")
            })
            .unwrap_or(4000);
...
fn get_metrics_config() -> Result<MetricsConfig, String> {
    ...
    let config_var = env::var("PAYCHAINS_METRICS_CONFIG")
        .map_err(|err| format!("PAYCHAINS_METRICS_CONFIG: {}", err))?;

    for pair in config_var.split(',') {
        let nv: Vec<_> = pair.split('=').collect();
        if nv.len() != 2 {
            return Err(format!("PAYCHAINS_METRICS_CONFIG is invalid: '{}'", pair));
        }
        ...
        match nv[0] {
            ...
            "u" => config.username = v,
            "p" => config.password = v,
            _ => return Err(format!("PAYCHAINS_METRICS_CONFIG is invalid: '{}'", pair)),
        }
    }

    if !config.complete() {
        return Err("PAYCHAINS_METRICS_CONFIG is incomplete".to_string());
    }
    Ok(config)
}
