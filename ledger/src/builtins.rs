use {
    paychains_runtime::builtins::{ActivationType, Builtin, Builtins},
...
fn genesis_builtins(bpf_jit: bool) -> Vec<Builtin> {
    ...
    // Windows: https://github.com/paychains-labs/rbpf/issues/217
    ...
    vec![
        to_builtin!(paychains_bpf_loader_deprecated_program!()),
        if bpf_jit {
            to_builtin!(paychains_bpf_loader_program_with_jit!())
        } else {
            to_builtin!(paychains_bpf_loader_program!())
        },
        if bpf_jit {
            to_builtin!(paychains_bpf_loader_upgradeable_program_with_jit!())
        } else {
            to_builtin!(paychains_bpf_loader_upgradeable_program!())
        },
    ]
}
