...
lazy_static! {
    ...
    ///
    /// > `~/.config/paychains/cli/config.yml`
    ///
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "paychains", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The PayChains CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a PayChains validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// PayChains documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://docs.paychains.com/cluster/rpc-endpoints
    ...
    /// documentation for `paychains_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `paychains_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from PayChains addresses to human-readable names.
    ///
    ...
    /// `paychains_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend(&[".config", "paychains", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.paychains.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.paychains.com"),
            "ws://api.devnet.paychains.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.paychains.com"),
            "wss://api.devnet.paychains.com/".to_string()
        );
