...
use {
    ...
    paychains_core::serve_repair::RepairProtocol,
...
fn get_repair_contact(nodes: &[ContactInfo]) -> ContactInfo {
    ...
    contact.id = paychains_sdk::pubkey::new_rand();
    contact
}
...
fn run_dos(
    ...
    match data_type.as_str() {
        ...
        "transaction" => {
            let tx = paychains_perf::test_tx::test_tx();
            info!("{:?}", tx);
...
#[cfg(test)]
pub mod test {
    use {super::*, paychains_sdk::timing::timestamp};
